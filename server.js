...
async function runJob(jobId, b) {
  ...
  exec(cmd, { timeout: 120000 }, (err2, stdout, stderr) => {
    ...
    Promise.resolve()
      ...
      .then(att => { if (att) record.file_pdf = JSON.stringify([att]); return saveQuoteToNocoDB(record); })
  ...
  exec(cmd, { timeout: 120000 }, (err2, stdout, stderr) => {
    ...
    if (TABLE_HD) {
      ...
      Promise.resolve()
        ...
        .then(att => { if (att) record.file_pdf = JSON.stringify([att]); return saveToNocoDB(TABLE_HD, record); })
